			...
			<table class="table table-bordered" style='font-size:12px'>
				...
				<?php foreach($list->result() as $row){ 
					...
					<tr>
						...
						<td>
							<center>
								<a href="<?php echo site_url('users/sedekah/edit/'.$row->ilmu_id); ?>" class="btn btn-info btn-sm" ><i class='glyphicon glyphicon-edit'></i> Edit </a>
								<a href="<?php echo site_url('users/hapus_sedekah/'.encode($row->ilmu_id))?>" class="btn btn-warning btn-sm" onclick="return confirm('Apakah anda yakin akan menghapus data ini ?')" ><i class='glyphicon glyphicon-trash'></i> Delete </a>
							</center>
						</td>
					</tr>
				<?php $no++; } ?>
